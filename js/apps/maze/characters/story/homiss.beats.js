...
export function homissBeats(H){
  ...
  { char: "homiss", depth: 4, make: () => ({
      ...
      node: { text: "*He does a fierce amount of tunin' before he says it.* ...here. Hypothetical, like. If ye ever came across a, a DOOR, say. Out. Not that there's an 'out' of a normal Tuesday, but sayin' there was... *the tunin' stops* ...ye'd mention it to a fella. Wouldn't ye. Not for ME. For a friend of mine. He's shy. Been in the one room a long time, an' his legs do be forgettin' what they're FOR. *The pretence hangs off him by a thread, an' he holds it anyway.* Ye'd mention it. That's all I'm askin'.",
        choices: [
  ...
  { char: "homiss", depth: 9,
    ...
    make: () => ({
      ...
      effects: { like: +1 },
      node: { text: "*He nods at what you're carrying.* ...is that a knuckle? *He holds his plectrum up next to it.* Snap, wha'. Fella sold me this one swore blind it came off a saint. Coddin' me, I'd say. *He looks at yours a moment longer than he means to.* ...I'd NEARLY say it. I'd not go wavin' that around. There's a man up the way would sell his own ma for the like of it." } }) },
  ...
  { char: "homiss", depth: 6,
    ...
    make: () => ({
      ...
      effects: { like: +1 },
      node: { text: "*The hummin' stops the moment ye stop walkin'.* Grand day! Grand... LISTEN. *He's at the glass in one step.* A wee bird — by which I mean I heard the wee man SHOUTIN' about it two floors up — says there's a JAR in circulation. *A reverent hush.* The white gold. The only condiment with a SOUL. *He grips the window frame.* I'm not askin' ye to do anythin'. I'm only sayin': there's a man at this window with savin's, prospects, an' a MORAL CLAIM. If that jar was to wander down the stairs... he'd remember it to his dyin' day. Which down here could be a long an' grateful time.",
        choices: [
          ...
          { text: "Scally's asking thirty-five tokens for it. Start saving.", effects: { like: +1 },
            next: { text: "THIRTY-F... *he does the sums out loud, appalled an' committed in the same breath* ...ROBBERY. Extortion of a man's SOUL through his sandwiches. I'll pay it. Obviously I'll pay it — but let it be NOTED I'll pay it FURIOUS. *He turns out his pockets: a plectrum, lint with promise.* ...how many tokens d'ye reckon a napkin fetches? Askin' for me. Not for a friend. ME." } },
          { text: "It's mayonnaise, Homiss. It's eggs and oil. Have some dignity.", effects: { like: -4 },
            next: { text: "*He recoils like ye've spat on the bass.* EGGS an' OIL?! That's like sayin' music is AIR WOBBLIN'. *A tremblin' finger.* Emulsification is the closest thing to a MIRACLE the kitchen ever produced. Two things that HATE each other, holdin' together, smooth as a hymn. If that's not worth wantin' in a place like THIS, I don't know why either of us is still talkin'. *He turns to the wall.* ...the WALL wouldn't have said it. An' the wall's said some quare things." } },
        ] } }) },
  ...
  { char: "homiss", depth: 7,
    ...
    make: () => ({
      ...
      node: { text: "*The plucking stops. He laughs, an' the laugh forgets what it was doin' halfway through.* ...Tuesday. Aye. That'd be Dalypso. Standin' there with the good extension lead, ragin'. *He counts nothin' on his fingers, a long while.* ...here. Which Tuesday would that be, now? Because the days do be slippy, down... AROUND here... *he stops himself, careful an' small now.* Tell him I'm sorry. I'll be at the next one, first round's on me. An' tell him he was right to be cross. He's always right to be cross. One of the great constants, like the speed of light." } }) },
  ...
  { char: "homiss", depth: 11, make: () => ({
      ...
      node: { text: "*Ye catch him mid-medley: somethin' thunderous, somethin' sweet, what might be a football chant slowed to a hymn.* Ah! Perfect timin'. The REQUEST LINE. Every Friday — an' I've decided it's Friday — I used to take requests through the walls. Bee wanted the drones, for the brainwaves. Sian shouted for somethin' with TEETH. An' the big fella with the telly, God love him, requested THEME TUNES. On a doctoral bass. An' I'd PLAY them, because a request is a sacred thing. *He damps the strings, an' the quiet where the audience used to be is suddenly very large.* They can't shout up the line anymore. So. *The ask is gentle an' enormous.* You've stood at all their windows. Make their requests FOR them, an' I'll play the lot. An' Friday stays Friday a wee bit longer.",
        choices: [
          { text: "Drones for Bee. Teeth for Sian. And the snooker theme for Dalypso.", effects: { like: +2 },
            next: { text: "*He points the plectrum at ye like a conductor's baton.* The SNOOKER theme! *He's laughin' before the first note.* Oh, he'd be WEEPIN'. Told me once it was the sound of civilisation. *He plays it, an' it IS gorgeous on the bass, stately as a liner leavin' port. Then the drones, two floors up. Then somethin' with teeth, sent down the other way like a care package.* ...there. Broadcast complete. They'll not have heard a note. *He looks down the hall.* ...or they will. Sound does quare things in this buildin'. Either way, the request line stays OPEN. Same time next Friday. Yer the whole switchboard now, wha'." } },
  ...
  { char: "homiss", depth: 12, make: () => ({
      ...
      node: { text: "*Ye stand there a full half-minute before he lets the hand down.* ...gone. *He turns.* Last night. The pipes. One voice came BACK, for a minute. Far off, down deep. An' before ye get excited: no. Not one of ours — I'd know them through ten floors of concrete. This one was SINGIN'. Low. Slow. A tune I HALF-knew, like a thing ye learned as a child an' lost the middle of. *Warmth an' dread holdin' hands now.* Nobody down here sings, friend. I've BEGGED them. So the question I can't put down: who's below us... an' HOW do they know a tune that I know?",
        choices: [
          ...
          { text: "Maybe the maze is learning to sing. It's learned everything else.", effects: { like: +1 },
            next: { text: "*He goes grey, an' then, bein' Homiss, turns the horror over to look at the craft of it.* ...the maze. LEARNIN' it. From WHO, but? A tune has to come from SOMEWHERE. A tune's a made thing. *He stops, an' the worse thought arrives, an' he says it quiet.* ...unless it learned it from one of US. Pulled it out of somebody's head like a splinter, some night, an' it's been practisin'. *He shudders, honest as a dog.* God. Imagine bein' LEARNED from, in yer sleep. *He picks up the bass an' holds it like armour.* New house rule: the drones get played LOUDER. If somethin' down there's takin' lessons, it can learn somethin' with STRUCTURE. *The grin crawls back, defiant.* Forty minutes. One note. CHOKE on it, ye eerie wee copycat." } },
          { text: "Half-known tunes are just déjà vu with a melody. It's nothing.", effects: { like: -3 },
            next: { text: "*He takes his ear off the wall an' gives ye a long, level look.* ...'nothin'.' *Like a note played flat on purpose.* Friend. Music is me TRADE. When I half-know a tune, that's a professional findin' a filed document with the middle pages gone — an' the filin' system is me own head. *He turns back to the wall.* Somethin' down there has one of MY tunes, singin' it in the dark, in a buildin' where nobody sings, an' ye'd file that under DÉJÀ VU? *He waves ye off.* Away an' tell Bee it's nothin'. She'll draw ye a CHART of how nothin' it is. An' come back when yer ready to take the pipes seriously. They've been righter than the both of us all along." } },
        ] } }) },
      ...
      node: { text: "*He sets the bass down with the exaggerated calm of a man puttin' somethin' down so as not to throw it.* ...saw a thing last night. Through the walls. The big lad below us, doin' his air-bass routine at the glass. Grand. Lovely. EXCEPT. *He holds up one finger.* Danglin' off his strap, an' I'd know it at a HUNDRED yards, I carved the ANGLES on it: a certain plectrum. Bone. Mine. The one I gave YOU. *He folds his arms.* I'm not cross about the plectrum. That's a lie, I'm a wee bit cross. But here's the ACTUAL wound, friend: I gave ye that as a keepsake, an' it turned up as a TROPHY. Ye armed the OTHER SIDE of a sacred twenty-year argument with MY relics. He'll be UNBEARABLE at the reunion gig. He'll have it MOUNTED.",
        choices: [
          { text: "He treasures it BECAUSE it's yours, Homiss. It's not a trophy. It's a relic of you.", req: { attr: "charisma", level: 6 }, effects: { like: +2 },
            next: { text: "*He opens his mouth to argue, an' stops, an' ye watch the reframe go through him like sun through a window.* ...a relic. *He picks the bass back up, mollified against his will.* Of ME. In the rival's CAMP. Like carryin' a saint's finger into battle... *he catches the parallel an' laughs despite himself.* God. The wee man's bone, my plectrum. This whole buildin's just RELICS movin' between believers. *He plucks a thoughtful note.* ...he does treasure it, mind. He'd not have it on the STRAP otherwise — the strap's where his da's pick lives. *The crossness quietly leaves the premises.* ...tell him if he ever plays a note worthy of it, it'll be the plectrum's doin'. He'll RAGE. It'll be gorgeous. Away with ye, ye wee arms dealer." } },
          { text: "It got him through a bad level. That's what your things do. They hold people together.",
            next: { text: "*The crossness deflates about halfway, which for a grievance this well-rehearsed is a landslide.* ...a bad level. *He looks off toward downstairs.* Aye. He's been havin' them. The pipes said as much — less roarin' lately, an' the roarin' was always the healthy part of him. *He rubs his beard.* An' it HELD him. The plectrum. *He sighs, enormous an' theatrical an' mostly surrender.* ...that's the problem with makin' things: ye lose the vote on what they're FOR. Ye carve a plectrum an' it becomes a HANDRAIL for a big lad in a headset. Nobody asks the luthier. *He waves ye off, gruff an' soft at once.* Go on. An' tell him mind the EDGE off it. It's a player's tool, not a POCKET SAINT. ...God. It is, though. This buildin', I swear." } },
      ...
      node: { text: "*He doesn't look up from the fretboard.* ...here. Ye'll think I'm astray. I've played this exact bar, in this exact SPOT, before. Not a bar like it. IT. *He sets the bass across his knees.* Ye know the mark on a sheet — D.C., da capo? 'Back to the top, play it again.' The whole corridor's got one on it, friend. Round we go. An' every time round, somethin's a hair off. A note bent. A wall a foot to the left. Small enough ye'd talk yerself out of it. Big enough it's eatin' at me.",
        choices: [
          ...
          { text: "Then play it forward. Skip to the last verse.",
            next: { text: "*He huffs a laugh.* Can't skip a da capo, friend — that's the cruelty of the notation. Ye play the WHOLE thing back before ye reach the codas. *He picks the bass up.* But there IS a coda. A wee sign further down the page: 'jump HERE, an' finish.' *He nods through the floor.* Somebody has to walk to it an' play the endin'. Faster than the hummin' does. Away with ye. Mind the repeats." } },
        ] } }) },
