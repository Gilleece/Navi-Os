...
export function sianBeats(H){
  ...
  { char: "sian", depth: 5, make: () => ({
      ...
      node: { text: "Composin' a ticket, hai. Bug report. 'SUMMARY: player character, that's me, unable to exit designated window volume. STEPS TO REPRODUCE: exist. EXPECTED BEHAVIOUR: doors.' *He mimes hittin' enter, then deflates.* ...no submit button in here. The one piece of UI they forgot. *The idea arrives like a sunrise.* HERE. You. Yer headin' DOWN, right? Every build's got a dev room at the bottom. It's LAW. When ye find it, submit this. Priority ONE. 'Let the big lad out.' An', eh... mark it urgent, hai. Not that it's urgent. Mark it urgent.",
        choices: [
      ...
      node: { text: "DOUBLE DIGITS, hai! *He high-fives the inside of the glass; ye supply the outside.* Ten levels — that's act two, OFFICIALLY. An' act two's where a game shows ye its TRUE genre. Act one played like a walkin' sim: gorgeous fog, chatty NPCs, collect-the-shinies. But act two's servin' resource pressure. Isolation. Unreliable environment. *He stops pacin'.* That's survival horror in a walkin' sim's clothes. An' the thing about survival horror — *game-brain an' the fear under it workin' together now* — the resource they're rationin' is never the tokens. It's the PEOPLE. Ye lose people as ye go. Genre contract. *A beat.* So I'm formally requestin' a genre shift. Co-op comedy, hai. I'll take a RACIN' game at this point.",
        choices: [
          ...
          { text: "What would the racing game version of this place even look like?",
            next: { text: "*The question hits him like a defibrillator.* OH. The maze, but yer KARTIN' through it. Fog's a slipstream mechanic. Tokens are boost. The windows are yer PIT CREW: I'm wavin' the board, Bee's callin' tyre strategy, Homiss on the anthem, an' Dalypso's the race steward, contestin' EVERY overtake, his OWN included. Scally runs the merch stand. *He wipes an eye, buoyant again, an' entirely aware of what ye did.* ...aye. That's the game they should've built with all this fog. Someday, partner. GO. Yer in me racin' line." } },
        ] } }) },
  ...
  { char: "sian", depth: 7, make: () => ({
      ...
      effects: { like: +1 },
      node: { text: "Right, it's decided: yer me co-op partner. Official, hai. This game's balanced for two: you've the locomotion, I've the game sense, between us one complete player. *He's grinnin', but somethin' under it is pacin' like a dog before thunder.* An' as yer partner, I get intel, right? Ye've seen the other windows. Ye've seen... HER window. Bee's. *He drops the casual all at once, like armour comin' off.* Just tell me how she is. Straight, hai. I can take straight.",
        choices: [
          ...
          { text: "Co-op, is it? What do I get out of this arrangement?",
            next: { text: "*He counts off on his fingers, instantly himself again.* One: me encyclopaedic knowledge of every game mechanic since PONG, free. Two: pit crew privileges when Brenda rides again. Three: the craic, self-evidently premium. An' four: *he leans in, mock-solemn* a friend on the INSIDE of the walls. D'ye know how rare that is? The windows are all TAKEN. *He sticks a hand against the glass for a shake it can't complete.* Partners. Done. No backsies. Bindin' in Cavan law." } },
        ] } }) },
  ...
  { char: "sian", depth: 8, make: () => ({
      ...
      effects: { like: +1 },
      node: { text: "*The answer starts instant an' confident.* Sure that's easy, it's on the... *the gesture at the empty air dies half-made.* ...the session timer's gone. There's usually a wee clock, hai. Battery, time played, 'take a break, ye degenerate'. Standard. *He counts on his fingers, an' the counting slows like a man walkin' into cold water.* I started on a Friday evenin'. Thinkin', two hours, then chips. An'... *he looks at his hands like a build he didn't write* ...I've seen YOU what, eight, nine levels? An' a good bit before you. *A silence with somethin' underneath it. The grin arrives half a second late, like a stand-in.* Batteries must be class though! CLASS. Chips are gonna taste UNREAL.",
        choices: [
  ...
  { char: "sian", depth: 9, make: () => ({
      ...
      effects: { like: +1 },
      node: { text: "Patch notes! *He presents the scratched wall like a whiteboard at a stand-up.* Every level, what changed. One-point-four: fog got heavier. One-point-five: lights stutterin'. Fine. Props. But HERE. *He taps a cluster of angry marks.* The echo went. The session timer went. The pause menu went. The GUARDIAN went. See the pattern? *The enthusiasm has somethin' colder under it now.* Real games ADD features. This one's been REMOVIN' them. One at a time. Quiet, like. That's not a changelog. That's a countdown. An' I can't work out what it's countin' down TO.",
        choices: [
          { text: "Add it to the ticket. All of it. This is good evidence.", effects: { like: +2 },
            next: { text: "*He straightens like ye've saluted him.* EVIDENCE. Aye! That's the word. Not 'worryin''. EVIDENCE. *He annotates the wall with fresh energy.* When ye reach the dev room this goes in the report, word for word, scratch for scratch. 'Systematic feature removal, user-hostile, reproducible, PRIORITY ONE.' *He steps back, back in control of his sprint board.* They can ignore a feelin', hai. They can't ignore a CHANGELOG. The one sacred text we have." } },
          ...
          { text: "Or you're seeing patterns in wear and tear. Walls crack, Sian.", effects: { like: -3 },
            next: { text: "*He looks at the wall of tallies, then at you, an' somethin' behind the visor goes flint.* ...wear an' tear. *He taps one scratch cluster, hard.* Did the ECHO wear out? Did the pause menu CRACK? *His voice stays level, which for Sian is the alarmin' version.* QA lad at heart, me. Day one they teach ye: users report feelin's, the LOG reports facts. I logged it BECAUSE I didn't trust the feelin'. Now the log agrees with the feelin', an' ye want me to distrust both together? *He turns back to the wall.* ...check yer own walls, partner. See if they're crackin' in ALPHABETICAL ORDER. Then we'll talk about wear an' tear." } },
        ] } }) },
  ...
  { char: "sian", depth: 16, make: () => ({
      ...
      effects: { like: +1 },
      node: { text: "*One eye closed, lining the corridor up like a spirit level.* Hai. Big lad. Tell me I'm not the only one seein' it. This level — I've WALKED it. Not one like it. IT. Same crack in the ceilin', same wonky light, byte for byte. *Half admirin', half appalled.* They re-used the whole ASSET. Studios do it, no shame — ye run outta budget an' redress an old room. But ye FLIP it, hai. This is a straight copy-paste. Ye don't copy-paste a level unless ye've run clean out of world.",
        choices: [
  ...
  { char: "sian", depth: 21, make: () => ({
      ...
      node: { text: "*The light off him stutters, an' he watches it happen on the back of his own hand with professional fascination.* Hai. Don't panic — I'M not panickin', we're both not panickin'. Frame drops. In me EYES now, not just the world. *He flexes the hand, countin' the stutters.* This is end-of-life, big lad. I've SHIPPED end-of-life. Ye stop patchin', pull the servers, an' the last players get this exact look — the flicker, the sound cuttin' to mono, the fog comin' in SQUARE. The build stops pretendin' it'll be here tomorrow.",
        choices: [
